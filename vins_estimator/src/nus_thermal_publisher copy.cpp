...
bool waitForSubscribers(const ros::Publisher &pub_left,
                        const ros::Publisher &pub_right,
                        ...
                        double timeout_sec)
{
    ...
    while (ros::ok())
    {
        const bool image_ready = pub_left.getNumSubscribers() > 0 && pub_right.getNumSubscribers() > 0;
        const bool imu_ready = !publish_imu || pub_imu.getNumSubscribers() > 0;
        if (image_ready && imu_ready)
            return true;

        if ((ros::WallTime::now() - start).toSec() >= timeout_sec)
        {
            ROS_WARN_STREAM("Timed out waiting for subscribers. left_subs="
                            << pub_left.getNumSubscribers()
                            << " right_subs=" << pub_right.getNumSubscribers()
                            << " imu_subs=" << (publish_imu ? pub_imu.getNumSubscribers() : 0));
            return false;
        }
        ros::spinOnce();
        rate.sleep();
...
int main(int argc, char **argv)
{
    ...
    double stereo_stamp_tolerance = 0.005;
    double subscriber_wait_timeout = 10.0;
    ...
    pnh.param("stereo_stamp_tolerance", stereo_stamp_tolerance, 0.005);
    pnh.param("subscriber_wait_timeout", subscriber_wait_timeout, 10.0);
    ...
    if (max_frames > 0)
        end_index = std::min(pair_count, start + static_cast<size_t>(max_frames));

    if (publish_imu)
    {
        const size_t requested_start = start;
        const size_t requested_end = end_index;
        const double imu_start = imu_samples.front().stamp;
        const double imu_end = imu_samples.back().stamp;
        while (start < end_index &&
               (left_frames[start].stamp < imu_start || right_frames[start].stamp < imu_start))
        {
            ++start;
        }
        while (end_index > start &&
               (left_frames[end_index - 1].stamp > imu_end || right_frames[end_index - 1].stamp > imu_end))
        {
            --end_index;
        }
        if (start >= end_index)
        {
            ROS_ERROR_STREAM("No image pairs are covered by IMU range. imu=["
                             << imu_start << ", " << imu_end << "] requested_pairs=["
                             << requested_start << ", " << requested_end << ")");
            return 1;
        }
        if (start != requested_start || end_index != requested_end)
        {
            ROS_WARN_STREAM("Trim image pairs to IMU coverage. requested=["
                            << requested_start << ", " << requested_end << ") trimmed=["
                            << start << ", " << end_index << ") imu=["
                            << imu_start << ", " << imu_end << "]");
        }
    }
    ...
    ROS_INFO_STREAM("Publishing NUS thermal stereo sequence"
                    ...
                    << " pairs=" << (end_index - start)
                    << " playback_rate=" << playback_rate
                    << " stereo_stamp_tolerance=" << stereo_stamp_tolerance);

    const size_t publish_count_limit = end_index - start;
    waitForSubscribers(pub_left, pub_right, pub_imu, publish_imu, subscriber_wait_timeout);

    do
    {
        size_t left_index = start;
        size_t right_index = start;
        size_t published_count = 0;
        double last_image_stamp = left_frames[left_index].stamp;
        size_t imu_index = publish_imu ? firstImuIndexForImageTime(imu_samples, left_frames[left_index].stamp) : 0;

        while (ros::ok() && left_index < end_index && right_index < end_index &&
               published_count < publish_count_limit)
        {
            const double left_stamp = left_frames[left_index].stamp;
            const double right_stamp = right_frames[right_index].stamp;
            const double stamp_diff = left_stamp - right_stamp;
            if (std::abs(stamp_diff) > stereo_stamp_tolerance)
            {
                if (stamp_diff < 0.0)
                {
                    ROS_WARN_STREAM_THROTTLE(1.0, "Skip left image due to stereo stamp mismatch left="
                                                      << left_stamp << " right=" << right_stamp
                                                      << " diff=" << stamp_diff);
                    ++left_index;
                }
                else
                {
                    ROS_WARN_STREAM_THROTTLE(1.0, "Skip right image due to stereo stamp mismatch left="
                                                      << left_stamp << " right=" << right_stamp
                                                      << " diff=" << stamp_diff);
                    ++right_index;
                }
            ...
            const ros::Time stamp = ros::Time().fromSec(left_stamp);
            if (publish_imu)
            {
                while (imu_index < imu_samples.size() && imu_samples[imu_index].stamp <= left_stamp)
                {
                    pub_imu.publish(makeImuMsg(imu_samples[imu_index], imu_frame_id));
                    ++imu_index;
                }
            }
            sensor_msgs::ImagePtr left_msg = readMonoImage(left_frames[left_index].path, stamp, left_frame_id, rectifier, true);
            sensor_msgs::ImagePtr right_msg = readMonoImage(right_frames[right_index].path, stamp, right_frame_id, rectifier, false);
            ...
            last_image_stamp = left_stamp;
            ++left_index;
            ++right_index;
            ++published_count;

            if (left_index < end_index && published_count < publish_count_limit)
            {
                double dt = left_frames[left_index].stamp - last_image_stamp;
                if (dt < 0.0)
                    dt = 0.0;
        ...
        {
            ROS_ERROR_STREAM("No stereo image pairs were published. Check left/right timestamps or increase "
                             "stereo_stamp_tolerance. Current tolerance=" << stereo_stamp_tolerance);
        }
    } while (ros::ok() && loop);
